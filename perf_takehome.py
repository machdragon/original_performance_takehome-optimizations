    ...
    def build_kernel_general(
        ...
        if use_cross_round:
            ...
            if self.enable_unroll_8 and rounds == 8:
                ...
                for unroll_r in range(8):  # Python loop for codegen, not runtime
                    round = unroll_r
                    info = round_info[round]
...
class Tests(unittest.TestCase):
    ...
    def test_level4_precompute_correctness(self):
        """Test that precomputed level 4 values match reference"""
        random.seed(42)
        forest = Tree.generate(10)
        inp = Input.generate(forest, 256, 16)
        mem = build_mem_image(forest, inp)
        
        # Build kernel with level4 precompute enabled
        # Note: This test requires the level4 precompute changes to be in perf_takehome.py
        # For now, test with submission_perf_takehome if available, or skip
        try:
            from submission_perf_takehome import KernelBuilder as SubmissionKernelBuilder
            kb = SubmissionKernelBuilder(enable_debug=False, assume_zero_indices=True, max_special_level=4)
        except ImportError:
            # Fall back to local KernelBuilder (may not have level4 precompute yet)
        ...
        assert (
            machine.mem[inp_values_p : inp_values_p + len(inp.values)]
            == ref_mem[inp_values_p : inp_values_p + len(inp.values)]
        ), "Level 4 precompute produced incorrect output values"

    def test_level4_vselect_correctness(self):
        """Test vselect tree selection for level 4"""
        # This test verifies that build_vselect_tree correctly selects nodes for level 4
        random.seed(42)
        forest = Tree.generate(10)
        inp = Input.generate(forest, 256, 16)
        mem = build_mem_image(forest, inp)
        
        # Test with debug enabled to verify intermediate values
        kb = KernelBuilder(enable_debug=True, assume_zero_indices=True, max_special_level=4)
        kb.build_kernel(10, len(forest.values), 256, 16)
        ...
        assert (
            machine.mem[inp_values_p : inp_values_p + len(inp.values)]
            == ref_mem[inp_values_p : inp_values_p + len(inp.values)]
        ), "Level 4 vselect tree produced incorrect output values"

    def test_level4_integration(self):
        """Full kernel test with level 4 precompute enabled"""
        random.seed(42)
        # Test multiple configurations
        for forest_height in [8, 10]:
            for batch_size in [128, 256]:
                forest = Tree.generate(forest_height)
                inp = Input.generate(forest, batch_size, 16)
                mem = build_mem_image(forest, inp)
                
                kb = KernelBuilder(enable_debug=False, assume_zero_indices=True, max_special_level=4)
                kb.build_kernel(forest_height, len(forest.values), batch_size, 16)
                ...
                assert (
                    machine.mem[inp_values_p : inp_values_p + len(inp.values)]
                    == ref_mem[inp_values_p : inp_values_p + len(inp.values)]
                ), f"Level 4 integration failed for h={forest_height}, b={batch_size}"

    def test_level4_performance(self):
        """Performance regression test - should improve cycles"""
        cycles = do_kernel_test(10, 16, 256, max_special_level=4)
        # Just verify it runs and produces a cycle count
        assert cycles > 0, "Level 4 precompute should produce valid cycle count"
        print(f"Level 4 precompute cycles: {cycles}")
